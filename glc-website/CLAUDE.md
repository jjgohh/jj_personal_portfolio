...
**The logo.** Generated by `tools/make_logo.py`: a portal frame (navy column and beam)
standing on the ground (a cyan bar that runs wider than the frame on both sides). Three
rectangles on a 64-unit grid, no curves, no effects. Full suite in `src/brand/`, served at
`/assets/brand/`, wordmarks converted to real Archivo outlines so printers need no font.
Rebuild with `python3 tools/make_logo.py`.
...
steps the client must take. Never lead the brand with the bare initials "GLC": in Malaysia
that is the standard shorthand for a government-linked company, and one of this site's three
audiences is government procurement.
