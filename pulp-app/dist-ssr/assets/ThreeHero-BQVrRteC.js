...
function ThreeHero() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const key = new THREE.DirectionalLight(16769456, 1.15);
      key.position.set(3, 4, 3);
      scene.add(key);
      const fill = new THREE.HemisphereLight(14674141, 15326400, 0.55);
      scene.add(fill);
      const rim = new THREE.DirectionalLight(13130280, 0.5);
      rim.position.set(-4, -1, -2);
      scene.add(rim);
      const group = new THREE.Group();
      scene.add(group);
      const gelMat = new THREE.MeshPhysicalMaterial({
        color: new THREE.Color(13130280),
        metalness: 0,
        roughness: 0.13,
        transmission: 0.9,
        thickness: 1.5,
        ...
        attenuationColor: new THREE.Color(9058834),
        attenuationDistance: 0.9,
        envMapIntensity: 1.25,
        specularIntensity: 1
      });
      const capsule = new THREE.Mesh(new THREE.CapsuleGeometry(0.62, 0.7, 28, 48), gelMat);
      capsule.rotation.z = Math.PI * 0.12;
      group.add(capsule);
      const core = new THREE.Mesh(
        new THREE.CapsuleGeometry(0.32, 0.44, 16, 32),
        new THREE.MeshBasicMaterial({ color: 15971174, transparent: true, opacity: 0.16 })
      );
      core.rotation.z = Math.PI * 0.12;
      group.add(core);
      const shTex = (() => {
        ...
        rg.addColorStop(0, "rgba(60,40,20,0.42)");
      ...
      const motes = new THREE.Points(pg, new THREE.PointsMaterial({ color: 13208142, size: 0.022, transparent: true, opacity: 0.5, depthWrite: false }));
      scene.add(motes);
      function layout() {
        W = wrap.clientWidth;
        H = wrap.clientHeight;
      ...
      function maybeRun() {
        if (inView && !hidden && rafId == null) rafId = requestAnimationFrame(loop);
      }
      ...
      function loop() {
        if (!(inView && !hidden) || disposed) {
          rafId = null;
          return;
        }
        rafId = requestAnimationFrame(loop);
        const t = (performance.now() - t0) / 1e3;
        if (baseY === void 0) baseY = group.position.y;
        if (!reduce) {
          ...
          core.rotation.y = group.rotation.y;
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => {
        if (!disposed) wrap.style.opacity = "1";
      }));
      cleanup = () => {
  ...
  return /* @__PURE__ */ jsx("div", { id: "gl-wrap", ref: wrapRef, children: /* @__PURE__ */ jsx("canvas", { id: "gl", ref: canvasRef }) });
}
export {
  ThreeHero as default
};
